...
1. 数据类型自身的对齐值：char自对齐字节为1，short为2，int/float为4，double为8，指针根据平台不同略有差异，32为为4而64为8字节。
...
对于如下代码：[代码来源](http://www.cnblogs.com/clover-toeic/p/3853132.html)
...
在gcc 6.4.0以及Linux 4.13.0平台输出如下：

```
a    address:   0x7ffee90d1c4f
b    address:   0x7ffee90d1c4c
c    address:   0x7ffee90d1c48
d[0] address:   0x7ffee90d1c30
d[1] address:   0x7ffee90d1c38
s    address:   0x7ffee90d1c20
s.m2 address:   0x7ffee90d1c28
```


由此可以发现栈对齐方式直接为4字节对齐，并且不会进行拼凑（即把1字节char和2字节short放入同一个4字节中）。
